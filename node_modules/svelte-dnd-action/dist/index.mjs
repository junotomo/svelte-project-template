...
function dndzone(node, options) {
  ...
  return {
    update: function update(newOptions) {
      console.debug("pointer dndzone will update newOptions: ".concat(toString(newOptions)));
...
function initAria() {
  if (isOnServer) return null; // setting the dynamic alerts
...
function handleZoneFocus(e) {
  ...
  if (e.currentTarget !== focusedDz) {
    ...
    dispatchFinalizeEvent(e.currentTarget, targetItems, {
      trigger: TRIGGERS.DROPPED_INTO_ZONE,
      id: focusedItemId,
      source: SOURCES.KEYBOARD
    });
...
function dndzone$1(node, options) {
  ...
  function handleDragStart(e) {
    ...
    if (!config.autoAriaDisabled) {
      alertToScreenReader("Started dragging item ".concat(focusedItemLabel, ". Use the arrow keys to move it within its list ").concat(focusedDzLabel, ", or tab to another list in order to move the item into it"));
    }
  ...
  function configure(_ref2) {
    ...
    node.tabIndex = isDragging && (node === focusedDz || config.dropFromOthersDisabled || focusedDz && config.type !== dzToConfig$1.get(focusedDz).type) ? -1 : 0;
  ...
  var handles = {
    update: function update(newOptions) {
      console.debug("keyboard dndzone will update newOptions: ".concat(toString(newOptions)));
      configure(newOptions);
    },
    destroy: function destroy() {
...
export { SHADOW_ITEM_MARKER_PROPERTY_NAME, SOURCES, TRIGGERS, alertToScreenReader, dndzone$2 as dndzone, overrideItemIdKeyNameBeforeInitialisingDndZones };
